...
def install_sty(dest_root: Path | None = None) -> Path:
    ...
    src_v2 = tex_dir / "leantexv2.sty"
    src_v2_onefile = tex_dir / "leantexv2onefile.sty"
    if not src_v2.exists():
        raise TeXInstallError(f"missing style file: {src_v2}")
    if not src_v2_onefile.exists():
        raise TeXInstallError(f"missing style file: {src_v2_onefile}")

    root = dest_root.resolve() if dest_root else detect_texmfhome().resolve()
    dest_dir = root / "tex" / "latex" / "leantex"
    dest_dir.mkdir(parents=True, exist_ok=True)

    dest = dest_dir / "leantexv2.sty"
    dest_onefile = dest_dir / "leantexv2onefile.sty"
    shutil.copy2(src_v2, dest)
    shutil.copy2(src_v2_onefile, dest_onefile)
