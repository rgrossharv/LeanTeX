...
[ -f "$EXAMPLE/leantexv2.generated.tex" ]

printf '[full-test] asserting snippet outputs exist...\n'
[ -f "$EXAMPLE/LeanTeX/snippets/snippet_001.out.raw.txt" ]
[ -f "$EXAMPLE/LeanTeX/snippets/snippet_002.out.raw.txt" ]
[ -f "$EXAMPLE/LeanTeX/extracted.infoview.raw.txt" ]

printf '[full-test] asserting expected message routing...\n'
grep -q '\[info\]' "$EXAMPLE/LeanTeX/snippets/snippet_001.out.raw.txt"
grep -q '\[error\]' "$EXAMPLE/LeanTeX/snippets/snippet_002.out.raw.txt"
grep -q '=== Infoview State ===' "$EXAMPLE/LeanTeX/extracted.infoview.raw.txt"
...
if grep -q '\[error\]' "$EXAMPLE/LeanTeX/snippets/snippet_002.out.raw.txt"; then
  echo "[full-test] expected no errors in onefile snippet_002" >&2
  exit 1
fi
grep -q 'Goals accomplished' "$EXAMPLE/LeanTeX/snippets/snippet_002.out.raw.txt"
